...
(function( w ){
	...
	w.picturefill = function() {
		...
		for( var i = 0, il = ps.length; i < il; i++ ){
			...
			if( matches.length ){			
				...
				// Nicolas: Set data-src attribute to use with async loader
				var match = matches.pop()
				picImg.src =  match.getAttribute( "data-src" );
				picImg.setAttribute( "data-src", match.getAttribute( "data-src-lazy" ) );
	...
	// Nicolas: Don't run on resize, and we'll run on domready using JQuery
